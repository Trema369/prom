...
export default function EvasiveButton({ onRegister }: { onRegister: () => void }) {
  ...
  function tap(pointerX?: number, pointerY?: number) {
    ...
    const maxX = box && btn ? Math.max(120, box.width - btn.width) : 260;
    const maxY = 200;
    ...
    const dist = 160 + next * 40;
    setPos((p) => ({
      x: clamp(p.x + (dx / len) * dist + (Math.random() * 60 - 30), -maxX, maxX),
      y: clamp(p.y + (dy / len) * dist + (Math.random() * 50 - 25), -maxY, maxY),
    }));
  }
